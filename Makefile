...
QUALITY_REPORT ?= $(PROJECT_TMP)/quality-report.md
...
## quality-report: Generate an on-demand quality scorecard under tmp/
quality-report: $(PROJECT_TMP)
	@echo "# 🚀 gh-orbit Quality Scorecard ($(DATE))" > $(QUALITY_REPORT)
	@echo "" >> $(QUALITY_REPORT)
	@echo "## 📊 Complexity Heatmap (Top 10)" >> $(QUALITY_REPORT)
	@echo "" >> $(QUALITY_REPORT)
	@echo "| File | Function | Metric | Score |" >> $(QUALITY_REPORT)
	@echo "| :--- | :--- | :--- | :--- |" >> $(QUALITY_REPORT)
	@golangci-lint run --default=none -E gocognit,cyclop,maintidx,funlen --output.text.path=stdout ./... | \
		awk -f tools/quality-report.awk | \
		sort -t '	' -k1,1nr | \
		head -n 10 | \
		cut -f2- >> $(QUALITY_REPORT)
	@echo "" >> $(QUALITY_REPORT)
	@echo "## 🧪 Mutation Score Baseline" >> $(QUALITY_REPORT)
	@echo "" >> $(QUALITY_REPORT)
	@echo "Run \`make quality-mutation\` manually to update these metrics." >> $(QUALITY_REPORT)
	@echo "" >> $(QUALITY_REPORT)
	@echo "Report generated by \`make quality-report\`." >> $(QUALITY_REPORT)
	@echo "Quality report generated at $(QUALITY_REPORT)"
	@cat $(QUALITY_REPORT)
